     ...
     char **argv;
{
  ...
  for (argc--, argv++; argc; argc--, argv++) {
    ...
    default:
      printf("%s\n", version());
      printf("usage: osim4 -f image.ppm\n");
      printf("       osim4 -c (from camera)\n");
      exit(1);
    }
  }
  ...
  if ((gp6x = u_gp6x760_open()) == -1) {
    printf("ERROR GP6X760 is not available\n");
    onintr_exit(1);
  }
#endif
  meminit();
...
#define MAXNUMBER 1
  ...
  /**************************/
  {
	...
	reg[0] = 0x00000000; u_usb_space_reg_write(usb, &reg[0], C0_EVA32_01);
	reg[0] = 0x00000000; u_usb_space_reg_write(usb, &reg[0], C0_MODE);
	reg[0] = 0x00000001; u_usb_space_reg_write(usb, &reg[0], C0_SIN);
	reg[0] = 0x00000000; u_usb_space_reg_write(usb, &reg[0], C0_SCK);
	reg[0] = 0x00000001; u_usb_space_reg_write(usb, &reg[0], C0_SCK);
	reg[0] = 0x456789ab; u_usb_space_reg_write(usb, &reg[0], C0_IH5_0);
	reg[0] = 0x456789ab; u_usb_space_reg_write(usb, &reg[0], C0_SPIKE23_0);
	reg[0] = 0x00000000; u_usb_space_reg_write(usb, &reg[0], C0_DCK);
	reg[0] = 0x00000001; u_usb_space_reg_write(usb, &reg[0], C0_DCK);
	reg[0] = 0x00000000; u_usb_space_reg_write(usb, &reg[0], C0_OSEL1_0);
	reg[0] = 0x00000001; u_usb_space_reg_write(usb, &reg[0], C0_MODE);
	reg[0] = 0x0003ffff; u_usb_space_reg_write(usb, &reg[0], C0_CAPTURE);
	reg[0] = 0x00000003; u_usb_space_reg_write(usb, &reg[0], C0_EVA32_01);

	do {
	  u_usb_space_reg_read(usb, &reg[0], C0_CAPTURE);
	  u_usb_space_reg_read(usb, &reg[1], C0_XDIFF14_0);
	  u_usb_space_reg_read(usb, &reg[2], C0_XPLUS14_0);
	  u_usb_space_reg_read(usb, &reg[3], C0_SOUT);
	  printf("usb%d: C0_CAPTURE=%08.8x C0_XDIFF14_0=%08.8x C0_XPLUS14_0=%08.8x C0_SOUT=%08.8x\n", usb, reg[0], reg[1], reg[2], reg[3]);
	} while (reg[0]);
      }
    }
  }
