...
#define PRINT_CUDA_IDX(name,mat,size) \
	puts(name); \
	cudaDeviceSynchronize(); \
	print_matrix_idx <<< 1 , 1 >>> (mat,size); \
	cudaDeviceSynchronize(); \
	puts("");
...
__global__
void print_matrix( double *mat , ITR_SIZE rows , ITR_SIZE cols ){
	for( ITR_SIZE i = 0 ; i < rows ; i++ ){
		for( ITR_SIZE j = 0 ; j < cols ; j++ ){
			printf("%.3lf\t",mat[ j * rows  + i ] );
			if( j % 5 == 0 )
				printf("\n");
		}
		printf("\n");
...
__global__
void print_matrix_idx( float *mat , ITR_SIZE size){
		for( ITR_SIZE idx = 0 ; idx < size ; idx++ ){
			printf("%d\t:%.3f\t", idx , mat[idx] );
			if( idx % 5 == 0 )
					printf("\n");
		}
		printf("\n");
