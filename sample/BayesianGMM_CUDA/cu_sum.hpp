...
__global__ void cpy_sum( in_type *g_idata, out_type *g_odata, const unsigned int N , const unsigned int k , unsigned int ofs ){
	__shared__ out_type sdata[threadNum];
#if !defined(__CUDA_ARCH__) || (__CUDA_ARCH__ >= 300)
	unsigned int tid = threadIdx.x;
	unsigned int i = blockIdx.x * (blockDim.x * 2) + threadIdx.x;
	out_type mysum = ( i < N ) ? g_idata[ k * N + i + ofs ] : 0;
	if ( i +  blockDim.x < N) mysum += g_idata[ ( k * N + i + ofs ) +  blockDim.x  ]  ;
	sdata[tid] = mysum;
	__syncthreads();
...
__global__ void get_sum( float *dev_K_ , float *dev_r_, const unsigned int N , const unsigned int k , const unsigned int offset ){
	float sum = 0;
	for( unsigned int idx = offset ; idx < N ; idx++ ){
		sum += dev_r_[ k * N + idx ];
	}
	dev_K_[k] += sum;
}
...
#define first_sum( a , b , from_mat , col ,  cpy_func ) \
	cpy_func< a , float , double > <<< b / a , a >>> ( from_mat , dev_N_ , b , col , offset ); \
	offset += b; \
	size -= b ; \
	tmp_size = b/a; \
	threadNum = a ; 
...
#define SUM(row_size,col_size, from_mat , to_mat , cpy_func , sum_func ) \
	cudaMemset( to_mat , 0 , sizeof(float) * col_size ); \
	for( int col = 0 ; col < col_size ; col++ ){ \
		...
		while( size > 0 ){ \
			...
			if( size >= 16777216 ){ \
				first_sum(256,16777216,from_mat,col,cpy_func); \
			} \
			else if( size >= 2097152 ){ \
				first_sum(128,2097152,from_mat,col,cpy_func); \
			} \
			else if( size >= 1048576 ){ \
				first_sum(1024,1048576,from_mat,col,cpy_func); \
			} \
			else if( size >= 262144 ){ \
				first_sum(512,262144,from_mat,col,cpy_func); \
			} \
			else if( size >= 65536 ){ \
				first_sum(256,65536,from_mat,col,cpy_func ); \
			} \
			else if( size >= 16384){ \
				first_sum(128,16384,from_mat,col,cpy_func ); \
			} \
			else if( size >= 4096 ){ \
				first_sum(64 , 4096 ,from_mat,col,cpy_func); \
			} \
			else if( size >= 1024 ){ \
				first_sum(1024,1024 ,from_mat,col,cpy_func); \
			} \
			else{ \
			...
			add_sum <<<  1 , 1 >>> ( &to_mat[col] , dev_N_ , 1 , 0  ); \
		} \
	}
